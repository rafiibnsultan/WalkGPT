...
def get_default_conv_template(name: str = "default") -> Conversation:
    """Return a fresh copy of the requested conversation template."""
    if name not in conv_templates:
        raise ValueError(f"Unknown conversation template: {name}")
    return copy.deepcopy(conv_templates[name])
