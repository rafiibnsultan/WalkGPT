...
from .PAVE_dataset import PAVEDataset
...
def collate_fn(
    ...
    conv = conversation_lib.conv_templates['llava_v1'].copy() if conv_type == "llava_v1" else conv
    conv = conversation_lib.conv_templates["llava_llama_2"].copy() if conv_type == "llava_llama_2" else conv
...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        exclude_val=False,
        dataset="sem_seg||refer_seg||vqa||reason_seg||PAVE",
        sample_rate=[9, 3, 3, 1],
    ...
    def __getitem__(self, idx):
        # Hui fix
        if len(self.all_datasets) == 1:               # only PAVE
            data = self.all_datasets[0]
            local_idx = idx % len(data)               # wrap if someone overrides __len__
            output = data[local_idx]
            inference = False
        ...
        inference = False
        if len(data) == 0:
            raise IndexError("Attempted to sample from an empty dataset.")
