...
class LlavaMetaForCausalLM(ABC):
    ...
    #rafi
    def _is_lang_space(self, feats):
        try:
            return feats is not None and feats.dim() == 3 and feats.size(-1) == self.get_model().config.hidden_size
        except Exception:
            return False
    def encode_images(self, images, clip_resize_list=None, return_project=False):
        """
        Returns:
        image_features: B×N×H_llm  (goes into the LLM sequence)
        vit_attention_mask_for_llm: None or mask  (only for CLIP pixel path)
        pre_image_features: list of per-level features for downstream use
        """

        # === Case 2: caller provided TOKENS (B×N×H_llm) ===
        if images is not None and images.dim() == 3:
            image_features = images                     # already in language space
            vit_attention_mask_for_llm = None

            if return_project:
                # Provide a list of "levels" already in H_llm. Minimal = one level.
                pre_image_features = [image_features]
                # If your later code expects multiple levels:
                # pre_image_features = [image_features for _ in range(self.image_feature_scale_num)]
            else:
                pre_image_features = []

            return image_features, vit_attention_mask_for_llm, pre_image_features

        # === Original pixel paths (CLIP/SAM-inside-LLaVA) stay unchanged ===
        vision_tower = self.get_model().get_vision_tower()
        vit_attention_mask_for_llm = None
        ...
        if isinstance(vision_tower.vision_tower, _CLIPVisionModel):
            ...
            vit_attention_mask = torch.zeros(
                (images.shape[0], h, w), dtype=torch.float32, device=images.device
            )
            if clip_resize_list is None:
                clip_resize_list = [(h, w)] * images.shape[0]
            for i, size in enumerate(clip_resize_list):
                vit_attention_mask[i, : size[0], : size[1]] = 1
            with torch.no_grad():
                patch_size = 14
                ...
                ones = torch.ones(
                    ...
                    1,
                    dtype=flatten_vit_attention_mask.dtype,
                    device=flatten_vit_attention_mask.device,
                )
                flatten_vit_attention_mask = torch.cat(
                    (ones, flatten_vit_attention_mask), dim=-1
                )
            image_features, pre_image_features = vision_tower(
                images, attention_mask=flatten_vit_attention_mask
            )
        else:
            image_features, pre_image_features = self.get_model().get_vision_tower()(images)
        ...
        if return_project:
            pre_image_features = [
                self.get_model().out_mm_projector(f) if self.get_model().separate_mm_projector
                else self.get_model().mm_projector(f)
                for f in pre_image_features
            ]
            output_image_features = [
                self.get_model().out_mm_projector(image_features) if self.get_model().separate_mm_projector
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # Downsample visual tokens (already [B, N, H_llm]) to reduce memory
        n, l, c = image_features.shape
        p = int(l ** 0.5)
        assert p * p == l, f"Token count {l} is not square."

        t = 16  # target grid size (try 12 or 8 if still OOM)
        grid = image_features.permute(0, 2, 1).reshape(n, c, p, p).float()
        grid = F.interpolate(grid, size=(t, t), mode="bilinear", align_corners=False)
        image_features = grid.flatten(2).permute(0, 2, 1).to(dtype=image_features.dtype)

        if vit_attention_mask is None:
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds): #the image lengths are not the same
            ...
            if attention_mask is not None and labels is not None and new_labels is not None:
                ...
                for cur_attention_mask, cur_new_labels, cur_new_labels_align in zip(
                    ...
                    updated_attention_mask.append(cur_new_attention_mask)
                if updated_attention_mask:
            ...
            if (
                ...
                and new_attention_mask.shape[1] < new_input_embeds.shape[1]
            ):
                new_attn_mask_pad_left = torch.full(
                    (
                ...
                new_attention_mask = torch.cat(
                    (new_attn_mask_pad_left, new_attention_mask), dim=1
                )
                attention_mask = new_attention_mask
                assert new_attention_mask.shape == new_input_embeds.shape[:2]
